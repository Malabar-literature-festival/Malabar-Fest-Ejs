...
router.post('/payment-status/:user', async function (req, res, next) {
  try {
    const title = 'Malabar Literature Festival | Celebrating History, Language, and Culture';
    const metaTags = [
      {
        description: 'Join the Malabar Literature Festival MLF, a cultural gathering celebrating the rich histories, legacies, languages, and arts of Malabar and its historic connections with distant lands like Kayalpattinam, Gujarat, Lakshadweep, Andaman and Nicobar, Hadhramaut, Malaysia, and Africa. Explore literature through open forums, talks, and debates. Delve into the lifeworlds of Mappila and Dalit subalterns. Discover a unique literary experience with Book Plus.',
      },
      {
        keywords: 'Malabar Literature Festival, MLF, Malabar, Kozhikode Beach, Calicut Beach, Malabar culture, literature, history, languages, arts, Kayalpattinam, Gujarat, Lakshadweep, Andaman and Nicobar, Hadhramaut, Malaysia, Africa, Mappila, Dalit, Book Plus, Malayalam readers',
      },
      {
        author: 'Malabar Literature Festival ',
      },
    ...
    const ccaResponse = await axios.post('https://apitest.ccavenue.com/apis/servlet/DoWebTrans', postData);

    // Handle the response here
    console.log('Response from CCAvenue:', ccaResponse.data);

    // Check if the response is an error
    if (ccaResponse.data.includes('command is mandatory')) {
      console.error('CCAvenue returned an error: Command is mandatory.');
      // Handle the error response here
    } else {
      // Parse the response as JSON and continue with further processing
      const responseData = JSON.parse(ccaResponse.data);
